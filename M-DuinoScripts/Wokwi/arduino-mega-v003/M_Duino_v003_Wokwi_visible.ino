/*
  Wokwi-ready wrapper for the M-Duino Trigger Box Controller v003.
  This variant uses slower, more visible timings for simulator review.

  IMPORTANT
    - This file is for Arduino Mega / Wokwi simulation only.
  This variant uses slower, more visible timings for simulator review.
    - The production source of truth remains:
      M-DuinoScripts/M_Duino_v003/M_Duino_v003.ino
...
/*
  ...
  DEBUGGING NOTES
    ...
    - Spark-test mode now requires an explicit Trigger press
      while Arm is active. It no longer free-runs automatically.
...
/*
  Spark-test mode behavior:
    ...
    - DAQ always OFF
    - If Arm is active:
        a spark pulse starts only when Trigger is pressed
    - If Arm is inactive:
        All outputs are OFF

  This is more controlled than simply holding Spark continuously ON,
  and gives the operator explicit control over when the spark starts.
*/
void handleSparkTestMode() {
  ...
  // Start one spark pulse on each debounced Trigger press
  if (!sparkTestPulseActive && triggerPressed) {
    setSpark(true);
...
void printStartupConfiguration() {
  ...
  Serial.println("================================================");
  Serial.println("Trigger Box Controller v003 startup");
